...
def check_artifacts_in_sync(
    ...
    if out_of_sync and not _has_failure(failures, "artifacts_out_of_sync"):
        ...
        for key in out_of_sync:
            details.append(
                f"{labels.get(key, key)}={_format_mtime(status[key]['mtime'])}"
            )
        failures.append(
            {
                ...
                "evidence": (
                    "正文更新时间 "
                    f"{_format_mtime(manuscript_mtime)} 晚于：{', '.join(details)}"
                ),
...
def _build_resync_instructions(
    ...
    if missing:
        missing_paths = [
            status[key]["path"].as_posix() for key in missing if key in status
        ]
    ...
    lines.append(
        f"python tools/continuity_checks.py --date {run_date} --chapter {chapter_id}"
    )
    lines.append("```")
    lines.append("")
    return "\n".join(lines)
...
def main() -> int:
    ...
    if not chapter_path.exists():
        ...
        manuscript_placeholder_issues = [
            issue for issue in _scan_placeholders(chapter_text) if issue != "TBD"
        ]
                    ...
                    failures.append(
                        {
                            "rule": "chapter_id_mismatch",
                            "evidence": (
                                f"state_patch.meta.current_chapter={patch_current} 与目标章节 {chap} 不一致。"
                            ),
    ...
    if not plan_path.exists():
        ...
        if plan_placeholder_issues:
            ...
            placeholder_report_lines.append(
                f"- chapter_plan: FAIL（{', '.join(plan_placeholder_issues)}）"
            )
        else:
    ...
    if changelog_path.exists():
        ...
        if changelog_issues:
            ...
            placeholder_report_lines.append(
                f"- changelog: FAIL（{', '.join(changelog_issues)}）"
            )
        else:
