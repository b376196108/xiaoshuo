...
> Hard rule: 正文（不含标题）必须 >=3000 字符；不足则返工。
...
Write:
- `manuscript/{chap}.md` (first line must be `# {chap}`)
...
- `recap/chapter_summaries/{chap}.md`
- `recap/rolling_recap.md`
- `state/state_patch.json` (JSON object; delta only; `meta.current_chapter` must be string "{chap}")
- `runs/{run_date}/changelog.md`
...
This generates/updates `runs/{run_date}/qa_report.md` (first line `QA_RESULT: PASS/FAIL`).
...
def _first_line_has_title(line: str, chap: str) -> bool:
    return line.startswith(f"# {chap} — ") or line.startswith(f"# {chap} - ")
...
def _insert_title_in_manuscript(chap: str, title: str, text: str) -> tuple[str, bool, str]:
    ...
    if len(lines) > 1 and lines[1].startswith("## 第"):
        return text, False, ""
...
def _insert_title_in_summary(chap: str, title: str, text: str) -> tuple[str, bool]:
    ...
    if lines:
        if chapter_num is not None and lines[0].startswith(f"## 第{chapter_num}章："):
            return text, False
        if _first_line_has_title(lines[0], chap):
            return text, False
    heading = format_chapter_heading(chap, title)
