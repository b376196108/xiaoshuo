...
执行完请跑：python tools/guard_diff.py --step 1 --date {run_date} --chapter {chap} && python tools/check_placeholders.py --date {run_date} --chapter {chap}
...
写入：
- `manuscript/{chap}.md`（第一行必须是 `# {chap}`；第二行必须是 `## 《标题》`；标题必须与 chapter_plan.md 的 chapter_title 完全一致）
执行完请跑：python tools/guard_diff.py --step 2 --date {run_date} --chapter {chap} && python tools/check_placeholders.py --date {run_date} --chapter {chap}
...
要求：
- recap/chapter_summaries/{chap}.md 第一行必须是 "# {chap}｜《本章标题》"，标题必须与 chapter_plan.md 的 chapter_title 完全一致。
执行完请跑：python tools/guard_diff.py --step 3 --date {run_date} --chapter {chap} && python tools/check_placeholders.py --date {run_date} --chapter {chap}
...
manuscript/{chap}.md 第一行必须仍为：# {chap}
...
若 runs/{run_date}/changelog.md 已存在：必须保留原内容，只能在文末追加小节 ## PunchUp Edit Log（bullets<=8），禁止覆盖/删除历史内容
