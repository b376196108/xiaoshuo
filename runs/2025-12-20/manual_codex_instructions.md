...
> 只允许写入指定文件，禁止修改其他任何文件；如需改其他文件，必须先停止并说明原因。
> 遇到缺失文件或读取失败，请先停止并报告，不要擅自补写或猜测。
> 禁止使用占位符：不得出现 "???" / "TBD" / "待补全" / "TODO"。
...
- `state/state_patch.json`（JSON 对象，仅 delta；必须包含 `meta.current_chapter: "ch001"`）
- `runs/2025-12-20/changelog.md`
- `runs/2025-12-20/qa_report.md`（由 Step 4 生成）
...
写入：
- `manuscript/ch001.md`（第一行必须是 `# ch001`）
...
- `recap/chapter_summaries/ch001.md`
- `recap/rolling_recap.md`
- `state/state_patch.json`（JSON 对象，仅 delta；`meta.current_chapter` 必须是字符串 "ch001"）
- `runs/2025-12-20/changelog.md`
...
该命令会生成/更新 `runs/2025-12-20/qa_report.md`（首行 `QA_RESULT: PASS/FAIL`）。
...
- BodyTooShort：只扩写 `manuscript/ch001.md` 正文（不改其他文件）→ 重跑 Step3 → 再跑 QA。
- OutOfSync：查看 `runs/2025-12-20/resync_instructions.md` 并按其指引执行，然后重新跑 QA。
