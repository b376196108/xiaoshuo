...
- `manuscript/ch006.md`
- `recap/chapter_summaries/ch006.md`
- `recap/rolling_recap.md`
- `state/state_patch.json`（JSON 对象，仅 delta；必须包含 `meta.current_chapter: "ch006"`）
- `runs/2025-12-25/changelog.md`
- `runs/2025-12-25/qa_report.md`（由 Step 4 生成）
...
提供以下文件作为上下文：
- `AGENTS.md`
- `runs/2025-12-25/brief.md`
- `runs/2025-12-25/context_pack.md`
...
执行完请跑：python tools/guard_diff.py --step 1 --date 2025-12-25 --chapter ch006 && python tools/check_placeholders.py --date 2025-12-25 --chapter ch006
...
写入：
- `manuscript/ch006.md`（第一行必须是 `# ch006`；第二行必须是 `## 《标题》`；标题必须与 chapter_plan.md 的 chapter_title 完全一致）
执行完请跑：python tools/guard_diff.py --step 2 --date 2025-12-25 --chapter ch006 && python tools/check_placeholders.py --date 2025-12-25 --chapter ch006
...
- `recap/chapter_summaries/ch006.md`
- `recap/rolling_recap.md`
- `state/state_patch.json`（JSON 对象，仅 delta；`meta.current_chapter` 必须是字符串 "ch006"）
- `runs/2025-12-25/changelog.md`

要求：
- recap/chapter_summaries/ch006.md 第一行必须是 "# ch006｜《本章标题》"，标题必须与 chapter_plan.md 的 chapter_title 完全一致。
执行完请跑：python tools/guard_diff.py --step 3 --date 2025-12-25 --chapter ch006 && python tools/check_placeholders.py --date 2025-12-25 --chapter ch006
...
manuscript/ch006.md 第一行必须仍为：# ch006
...
若 runs/2025-12-25/changelog.md 已存在：必须保留原内容，只能在文末追加小节 ## PunchUp Edit Log（bullets<=8），禁止覆盖/删除历史内容
...
该命令会生成/更新 `runs/2025-12-25/qa_report.md`（首行 `QA_RESULT: PASS/FAIL`）。
若 `QA_RESULT: PASS`，再执行：
```bash
python tools/merge_state_patch.py --date 2025-12-25
```

## 常见失败与处理
- BodyTooShort：只扩写 `manuscript/ch006.md` 正文（不改其他文件）→重跑 Step3 →再跑 QA。
- OutOfSync：查看 `runs/2025-12-25/resync_instructions.md` 并按其指引执行，然后重跑 QA。
- TitleMissingOrInvalid：修正正文标题为 `# ch006` + `## 《标题》`，标题来自 chapter_plan.md 的 chapter_title →重跑 Step3 →再跑 QA。
