...
- `state/state_patch.json`（JSON 对象，仅 delta；必须包含 `meta.current_chapter: "ch002"`）
- `runs/2025-12-21/changelog.md`
- `runs/2025-12-21/qa_report.md`（由 Step 4 生成）
...
写入：
- `manuscript/ch002.md`（第一行必须是 `# ch002`；第二行必须是 `## 《标题》`；标题必须与 chapter_plan.md 的 chapter_title 完全一致）
...
- `recap/chapter_summaries/ch002.md`
- `recap/rolling_recap.md`
- `state/state_patch.json`（JSON 对象，仅 delta；`meta.current_chapter` 必须是字符串 "ch002"）
- `runs/2025-12-21/changelog.md`

要求：
- recap/chapter_summaries/ch002.md 第一行必须是 "# ch002｜《本章标题》"，标题必须与 chapter_plan.md 的 chapter_title 完全一致。
...
该命令会生成/更新 `runs/2025-12-21/qa_report.md`（首行 `QA_RESULT: PASS/FAIL`）。
...
## 常见失败与处理
- BodyTooShort：只扩写 `manuscript/ch002.md` 正文（不改其他文件）→重跑 Step3 →再跑 QA。
- OutOfSync：查看 `runs/2025-12-21/resync_instructions.md` 并按其指引执行，然后重跑 QA。
- TitleMissingOrInvalid：修正正文标题为 `# ch002` + `## 《标题》`，标题来自 chapter_plan.md 的 chapter_title →重跑 Step3 →再跑 QA。
