...
## 需要产出（路径）
- `runs/2025-12-23/chapter_plan.md`
- `manuscript/ch004.md`
- `recap/chapter_summaries/ch004.md`
- `recap/rolling_recap.md`
- `state/state_patch.json`（JSON 对象，仅 delta；必须包含 `meta.current_chapter: "ch004"`）
- `runs/2025-12-23/changelog.md`
- `runs/2025-12-23/qa_report.md`（由 Step 4 生成）
...
提供以下文件作为上下文：
- `AGENTS.md`
- `runs/2025-12-23/brief.md`
- `runs/2025-12-23/context_pack.md`
...
写入：
- `manuscript/ch004.md`（第一行必须是 `# ch004`；第二行必须是 `## 《标题》`；标题必须与 chapter_plan.md 的 chapter_title 完全一致）
...
- `recap/chapter_summaries/ch004.md`
- `recap/rolling_recap.md`
- `state/state_patch.json`（JSON 对象，仅 delta；`meta.current_chapter` 必须是字符串 "ch004"）
- `runs/2025-12-23/changelog.md`

要求：
- recap/chapter_summaries/ch004.md 第一行必须是 "# ch004｜《本章标题》"，标题必须与 chapter_plan.md 的 chapter_title 完全一致。
...
在仓库根目录执行：
```bash
python tools/continuity_checks.py --date 2025-12-23 --chapter ch004
```

该命令会生成/更新 `runs/2025-12-23/qa_report.md`（首行 `QA_RESULT: PASS/FAIL`）。
若 `QA_RESULT: PASS`，再执行：
```bash
python tools/merge_state_patch.py --date 2025-12-23
```

## 常见失败与处理
- BodyTooShort：只扩写 `manuscript/ch004.md` 正文（不改其他文件）→重跑 Step3 →再跑 QA。
- OutOfSync：查看 `runs/2025-12-23/resync_instructions.md` 并按其指引执行，然后重跑 QA。
- TitleMissingOrInvalid：修正正文标题为 `# ch004` + `## 《标题》`，标题来自 chapter_plan.md 的 chapter_title →重跑 Step3 →再跑 QA。
